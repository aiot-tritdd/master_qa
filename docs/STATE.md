...
- ✅ **Type-track trên Pro + Reservation** (2026-07-17) — lần đầu ra ngoài ticket-app:
  ...
  - **security giai đoạn 1** (6 họ **read-only**, không ghi data lên dev chung): headers **FAIL cả 3 màn** → **BUG-035** (Pro thiếu CẢ 5 header) + **BUG-036** (Reservation thiếu cả 5 — widget CÔNG KHAI nên thiếu X-Frame-Options = clickjack được luồng đặt lịch). CORS/error-disclosure/open-redirect/force-browse **PASS**. 7 họ mutating + IDOR = **未実施** (lý do thật ghi trong results, không bịa PASS).
  ...
  - **Sổ bug: 20 → 36.** Không tạo data rác (toàn read-only) → không cần `/testcase-cleanup`.
