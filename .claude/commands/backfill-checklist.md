...
2. **Lấy staff code** từ `DEV-ACCOUNTS.md` (dòng institute_code đó): `pro_staff_code` + `ticket_staff_code` (thường `seed-admin`).
...
7. **Báo user:** tóm tắt config (branch, chiều, candidates, noop) + đường dẫn CHECKLIST.md. **KHÔNG chạy /backfill-run** — chờ user xem checklist + ra lệnh.
